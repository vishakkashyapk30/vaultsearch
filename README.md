...
row index maps directly to `(chunk_id, doc_id, title, text, allowed_principals)`. The ACL rides physically alongside the vector — the
...
**4b. BM25 over the allowed subset.** `rank_bm25`'s `get_batch_scores(query_tokens, allowed_ids)` scores *only* the allowed indices. Top-30 with positive scores
survive.
...
Open **[http://localhost:8000](http://localhost:8000)** in your browser. Pick a person on the left, ask
...
Then open [http://localhost:8000](http://localhost:8000). The container builds the data and indexes on
...
│   ├── agents.py                   Orchestrator, QueryPlanner, PermissionVerifier,
│   │                               AnswerSynthesizer, citation sanitizer
...
├── docker-compose.yml              one-command local deployment (Ollama on host)
...
| LLM            | Ollama (default `gemma3:4b`)             | query planning and grounded answer synthesis, fully local       |
| Frontend       | Plain HTML / CSS / JS                    | zero-build interface served by the API process                  |
| Testing        | pytest                                   | ACL predicate first, then ingestion, retrieval, verification    |
| Packaging      | Docker + docker-compose                  | reproducible deployment, no cloud account required              |
